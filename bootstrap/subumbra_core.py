...
BUILTIN_CONSUMER_IDS = tuple(ADAPTER_SCOPE_VARS.keys())
...
def _write_keys_payload(keys_payload: dict[str, Any]) -> None:
    ...
    try:
        ...
        die(f"Failed to write endpoint.json: {exc}")
...
def _build_runtime_env_lines(
    *,
    ...
    janus_vapid_public_key: str = "",
    ui_username: str = "",
    ui_password_hash: str = "",
    cf_access_protected: bool = False,
    deploy_ui: bool = False,
    deploy_ssh: bool = False,
) -> list[str]:
    runtime_env_lines = [
        f"# Generated by subumbra-bootstrap on {now_iso}",
        "# PRIVILEGED — treat like an API key; restrict access to this file",
        f"SUBUMBRA_CONSUMER_REGISTRY={json.dumps(consumer_registry, separators=(',', ':'))}",
        f"PROXY_ALLOWED_KEYS={','.join(allowed_keys_by_consumer['subumbra-proxy'])}",
        f"UI_ALLOWED_KEYS={','.join(allowed_keys_by_consumer['subumbra-ui'])}",
        f"SUBUMBRA_TOKEN_PROXY={consumer_tokens['subumbra-proxy']}",
        f"SUBUMBRA_TOKEN_UI={consumer_tokens['subumbra-ui']}",
        f"SUBUMBRA_MANAGEMENT_TOKEN={management_token}",
    ]
    if "subumbra-probe" in allowed_keys_by_consumer:
        runtime_env_lines.append(
            f"PROBE_ALLOWED_KEYS={','.join(allowed_keys_by_consumer['subumbra-probe'])}"
        )
        runtime_env_lines.append(
            f"SUBUMBRA_TOKEN_PROBE={consumer_tokens['subumbra-probe']}"
        )
    for consumer_id in allowed_keys_by_consumer:
        if consumer_id not in BUILTIN_CONSUMER_IDS:
            runtime_env_lines.append(
                f"SUBUMBRA_TOKEN_{_normalize_consumer_id(consumer_id)}={consumer_tokens[consumer_id]}"
            )
    runtime_env_lines.extend(
        [
            f"SUBUMBRA_HMAC_KEY={subumbra_hmac_key}",
            f"CF_WORKER_URL={worker_url}",
            f"SUBUMBRA_JANUS_VAPID_PUBLIC_KEY={janus_vapid_public_key}",
            f"UI_USERNAME={ui_username}",
            f"UI_PASSWORD_HASH={ui_password_hash}",
            f"CF_ACCESS_PROTECTED={'true' if cf_access_protected else 'false'}",
            f"DEPLOY_UI={'true' if deploy_ui else 'false'}",
            f"DEPLOY_SSH={'true' if deploy_ssh else 'false'}",
...
def _build_host_env_updates(
    *,
    ...
    host_env_updates = {
        "SUBUMBRA_CONSUMER_REGISTRY": json.dumps(consumer_registry, separators=(",", ":")),
        "PROXY_ALLOWED_KEYS": ",".join(allowed_keys_by_consumer["subumbra-proxy"]),
        "UI_ALLOWED_KEYS": ",".join(allowed_keys_by_consumer["subumbra-ui"]),
        "SUBUMBRA_TOKEN_PROXY": consumer_tokens["subumbra-proxy"],
        "SUBUMBRA_TOKEN_UI": consumer_tokens["subumbra-ui"],
        ...
        "SUBUMBRA_JANUS_VAPID_PUBLIC_KEY": janus_vapid_public_key,
        "UI_USERNAME": ui_username,
        "UI_PASSWORD_HASH": ui_password_hash,
        "CF_ACCESS_PROTECTED": "true" if cf_access_protected else "false",
        "DEPLOY_UI": "true" if deploy_ui else "false",
        "DEPLOY_SSH": "true" if deploy_ssh else "false",
    }
    if "subumbra-probe" in allowed_keys_by_consumer:
        host_env_updates["PROBE_ALLOWED_KEYS"] = ",".join(allowed_keys_by_consumer["subumbra-probe"])
        host_env_updates["SUBUMBRA_TOKEN_PROBE"] = consumer_tokens["subumbra-probe"]
    for consumer_id in allowed_keys_by_consumer:
        if consumer_id not in BUILTIN_CONSUMER_IDS:
            host_env_updates[f"SUBUMBRA_TOKEN_{_normalize_consumer_id(consumer_id)}"] = consumer_tokens[consumer_id]
    if cf_runtime_creds:
...
def _normalize_policy_doc(doc: dict[str, Any], source: str) -> dict[str, Any]:
    ...
    if gate is not None:
        ...
        if not isinstance(require_approval, list) or not require_approval:
            _policy_die(source, "gate.require_approval must be a non-empty array")
        normalized_rules: list[dict[str, Any]] = []
        for idx, raw_rule in enumerate(require_approval):
            if not isinstance(raw_rule, dict):
                _policy_die(source, f"gate.require_approval[{idx}] must be an object")
            timeout_seconds = raw_rule.get("timeout_seconds")
            if (
                not isinstance(timeout_seconds, int)
                or isinstance(timeout_seconds, bool)
                or timeout_seconds <= 0
            ):
                _policy_die(source, f"gate.require_approval[{idx}].timeout_seconds must be a positive integer")
            raw_when = raw_rule.get("when")
            if not isinstance(raw_when, dict):
                _policy_die(source, f"gate.require_approval[{idx}].when must be an object")
            normalized_when: dict[str, Any] = {}
            if raw_when.get("any_request") is True:
                normalized_when["any_request"] = True
            elif "any_request" in raw_when:
                _policy_die(source, f"gate.require_approval[{idx}].when.any_request must be true when provided")
            consumer = raw_when.get("consumer")
            if consumer is not None:
                consumer = _policy_require_string(consumer, source, f"gate.require_approval[{idx}].when.consumer")
                if not CONSUMER_ID_RE.fullmatch(consumer):
                    _policy_die(source, f"gate.require_approval[{idx}].when.consumer {consumer!r} is invalid")
                normalized_when["consumer"] = consumer
            method = raw_when.get("method")
            if method is not None:
                method = _policy_require_string(method, source, f"gate.require_approval[{idx}].when.method")
                if method not in POLICY_ALLOWED_METHODS:
                    _policy_die(source, f"gate.require_approval[{idx}].when.method {method!r} is invalid")
                normalized_when["method"] = method
            path_prefix = raw_when.get("path_prefix")
            if path_prefix is not None:
                path_prefix = _policy_require_string(path_prefix, source, f"gate.require_approval[{idx}].when.path_prefix")
                if not path_prefix.startswith("/"):
                    _policy_die(source, f"gate.require_approval[{idx}].when.path_prefix must start with '/'")
                normalized_when["path_prefix"] = path_prefix
            if not normalized_when:
                _policy_die(source, f"gate.require_approval[{idx}].when must declare at least one match condition")
            unknown_when = set(raw_when) - {"any_request", "consumer", "method", "path_prefix"}
            if unknown_when:
                _policy_die(
                    source,
                    f"gate.require_approval[{idx}].when contains unsupported field(s): {', '.join(sorted(unknown_when))}",
                )
            unknown_rule = set(raw_rule) - {"when", "timeout_seconds"}
            if unknown_rule:
                _policy_die(
                    source,
                    f"gate.require_approval[{idx}] contains unsupported field(s): {', '.join(sorted(unknown_rule))}",
                )
...
def _load_keys_payload_or_die() -> dict[str, dict[str, Any]]:
    ...
    if not isinstance(payload, dict):
        die("endpoint.json is malformed")
    return payload
...
def _normalize_manifest_record(record: Any, idx: int) -> dict[str, Any]:
    ...
    if record_type == "ssh_key":
        ...
        if gate is not None:
            ...
            require_approval = gate.get("require_approval")
            if not isinstance(require_approval, list) or not require_approval:
                _manifest_die(f"{source}.gate.require_approval must be a non-empty array")
            normalized_rules: list[dict[str, Any]] = []
            for rule_idx, raw_rule in enumerate(require_approval):
                if not isinstance(raw_rule, dict):
                    _manifest_die(f"{source}.gate.require_approval[{rule_idx}] must be an object")
                timeout_seconds = raw_rule.get("timeout_seconds")
                if (
                    not isinstance(timeout_seconds, int)
                    or isinstance(timeout_seconds, bool)
                    ...
                    _manifest_die(
                        f"{source}.gate.require_approval[{rule_idx}].timeout_seconds must be a positive integer"
                    )
                raw_when = raw_rule.get("when")
                if not isinstance(raw_when, dict):
                    _manifest_die(f"{source}.gate.require_approval[{rule_idx}].when must be an object")
                normalized_when: dict[str, Any] = {}
                if raw_when.get("any_request") is True:
                    normalized_when["any_request"] = True
                elif "any_request" in raw_when:
                    _manifest_die(
                        f"{source}.gate.require_approval[{rule_idx}].when.any_request must be true when provided"
                    )
                consumer = raw_when.get("consumer")
                if consumer is not None:
                    if not isinstance(consumer, str) or not consumer.strip():
                        _manifest_die(
                            f"{source}.gate.require_approval[{rule_idx}].when.consumer must be a non-empty string"
                        )
                    if not CONSUMER_ID_RE.fullmatch(consumer.strip()):
                        _manifest_die(
                            f"{source}.gate.require_approval[{rule_idx}].when.consumer {consumer.strip()!r} is invalid"
                        )
                    normalized_when["consumer"] = consumer.strip()
                if "method" in raw_when or "path_prefix" in raw_when:
                    _manifest_die(
                        f"{source}.gate.require_approval[{rule_idx}] cannot use method/path_prefix for ssh_key records"
                    )
                if not normalized_when:
                    _manifest_die(
                        f"{source}.gate.require_approval[{rule_idx}].when must declare at least one match condition"
                    )
                unknown_when = set(raw_when) - {"any_request", "consumer"}
                if unknown_when:
                    _manifest_die(
                        f"{source}.gate.require_approval[{rule_idx}].when contains unsupported field(s): "
                        f"{', '.join(sorted(unknown_when))}"
                    )
                unknown_rule = set(raw_rule) - {"when", "timeout_seconds"}
                if unknown_rule:
                    _manifest_die(
                        f"{source}.gate.require_approval[{rule_idx}] contains unsupported field(s): "
...
def _manifest_bool_value(value: Any, source: str) -> bool:
    if isinstance(value, bool):
        return value
    _manifest_die(f"{source} must be true or false")


def _load_manifest_service_config() -> dict[str, Any]:
    payload = _load_manifest_payload()
    ...
    if not isinstance(services, dict):
        _manifest_die("top-level 'services' must be an object when present")

    deploy_ui = False
    ui_auth_mode = ""
    ui_block = services.get("ui")
    ui_declared = ui_block is not None
    if ui_block is not None:
        if not isinstance(ui_block, dict):
            _manifest_die("services.ui must be an object when present")
        if "deploy" in ui_block:
            deploy_ui = _manifest_bool_value(ui_block["deploy"], "services.ui.deploy")
        auth_block = ui_block.get("auth")
        if auth_block is not None:
            if not isinstance(auth_block, dict):
                _manifest_die("services.ui.auth must be an object when present")
            ui_auth_mode = str(auth_block.get("mode", "")).strip().lower()
    if deploy_ui and ui_auth_mode not in UI_AUTH_MODES:
        _manifest_die("services.ui.auth.mode is required when services.ui.deploy is true")
    if ui_auth_mode and ui_auth_mode not in UI_AUTH_MODES:
        _manifest_die("services.ui.auth.mode must be one of: basic, cf_access, both")

    deploy_ssh = False
    ssh_block = services.get("ssh_agent")
    ssh_declared = ssh_block is not None
    if ssh_block is not None:
        if not isinstance(ssh_block, dict):
            _manifest_die("services.ssh_agent must be an object when present")
        if "deploy" in ssh_block:
            deploy_ssh = _manifest_bool_value(ssh_block["deploy"], "services.ssh_agent.deploy")
...
def _resolve_policy_for_key(
    ...
    if sorted(_policy_consumer_ids(policy)) != sorted(allowed_consumers):
        die(
            f"Policy consumer conflict for key_id {key_id!r}: "
            f"policy consumers={', '.join(_policy_consumer_ids(policy))} "
            f"do not match bootstrap consumers={', '.join(allowed_consumers)}"
        )
    return policy
...
def _require_fat_record_fields(record: dict[str, Any], key_id: str) -> tuple[dict[str, Any], list[str]]:
    ...
    if not isinstance(policy, dict):
        die(
            f"endpoint.json record {key_id!r} is missing embedded policy authority.\n"
            "  Repair the record or re-run full bootstrap."
        )
    consumers = record.get("consumers")
    if not isinstance(consumers, list) or not consumers or not all(isinstance(consumer_id, str) and consumer_id for consumer_id in consumers):
        die(
            f"endpoint.json record {key_id!r} is missing embedded consumer authority.\n"
            "  Repair the record or re-run full bootstrap."
        )
    return policy, list(consumers)


def _verify_embedded_policy_hash(record: dict[str, Any], key_id: str) -> None:
    policy, _consumers = _require_fat_record_fields(record, key_id)
    stored_policy_id = record.get("policy_id")
    if not isinstance(stored_policy_id, str) or not stored_policy_id.strip():
        die(
            f"endpoint.json record {key_id!r} is missing policy_id.\n"
            "  Repair the record or re-run full bootstrap."
        )
    ...
    if not isinstance(stored_policy_hash, str) or not stored_policy_hash.strip():
        die(
            f"endpoint.json record {key_id!r} is missing policy_hash.\n"
            "  Repair the record or re-run full bootstrap."
        )
        ...
        if record["key_id"] == target_key_id:
            if record.get("type") == "ssh_key":
                return {
                    ...
                    "consumers": list(record["effective_consumers"]),
...
def _append_unique_consumer_binding(
    key_consumers_by_key_id: dict[str, list[str]],
    key_id: str,
    consumer_id: str,
) -> None:
    bindings = key_consumers_by_key_id.setdefault(key_id, [])
    if consumer_id not in bindings:
        bindings.append(consumer_id)


def _prompt_app_label(prompt: str = "  App/label for this key: ") -> str:
    while True:
        app_label = input(prompt).strip().lower()
        if CONSUMER_ID_RE.fullmatch(app_label):
            return app_label
...
def _prompt_declared_consumer_ids() -> list[str]:
    print("\n" + "═" * 70)
    print("  Subumbra Bootstrap — Step 2 of 4: App Consumers")
    print("═" * 70)
    print("  Declare the app consumer IDs that should receive per-app Subumbra tokens.")
    print("  Example: litellm,openwebui")
    print("  Leave blank only if you intentionally want compatibility/simple mode.\n")

    while True:
        raw = input("  Declared app consumer IDs (comma-separated, blank = none): ").strip()
        try:
            return _parse_consumer_ids(raw)
        except SystemExit:
            print("  ✗  Invalid consumer declaration. Please try again.\n")


def _prompt_key_consumer_ids(key_id: str, declared_consumer_ids: list[str]) -> list[str]:
    if not declared_consumer_ids:
        info(f"{key_id} will use compatibility/simple mode because no app consumers were declared.")
        return []

    print(f"  Declared app consumers: {', '.join(declared_consumer_ids)}")
    print("  Enter comma-separated consumer IDs, 'all' for every declared app, or blank for compatibility/simple mode.")

    while True:
        raw = input(f"  Consumers for {key_id}: ").strip()
        if raw.lower() == "all":
            return list(declared_consumer_ids)
        try:
            return _parse_key_consumer_ids(
                raw,
                source=f"Interactive binding for {key_id}",
                declared_consumer_ids=set(declared_consumer_ids),
            )
        except SystemExit:
            print("  ✗  Invalid consumer selection. Please try again.\n")
...
def _normalize_consumer_id(consumer_id: str) -> str:
    return consumer_id.upper().replace("-", "_")


def _parse_consumer_ids(raw: str) -> list[str]:
    consumer_ids: list[str] = []
    seen_normalized: dict[str, str] = {}
    for consumer_id in (item.strip() for item in raw.split(",")):
        if not consumer_id:
            continue
        if not CONSUMER_ID_RE.fullmatch(consumer_id):
            die(f"Invalid consumer_id '{consumer_id}'")
        if consumer_id in BUILTIN_CONSUMER_IDS:
            die(f"Reserved built-in consumer_id '{consumer_id}'")
        normalized = _normalize_consumer_id(consumer_id)
        if normalized in BUILTIN_TOKEN_SUFFIXES:
            die(f"Reserved built-in consumer token suffix '{normalized}' for consumer_id '{consumer_id}'")
        if normalized in seen_normalized:
            die(
                f"Duplicate normalized consumer token suffix '{normalized}' for consumer_ids "
                f"'{seen_normalized[normalized]}' and '{consumer_id}'"
            )
        seen_normalized[normalized] = consumer_id
        consumer_ids.append(consumer_id)
    return consumer_ids


def _build_custom_adapter_scope_vars(consumer_ids: list[str]) -> dict[str, str]:
    return {
        consumer_id: f"{_normalize_consumer_id(consumer_id)}_ALLOWED_KEYS"
        for consumer_id in consumer_ids
    }


def _parse_key_consumer_ids(
    raw: str,
    *,
    source: str,
    declared_consumer_ids: set[str],
) -> list[str]:
    consumer_ids: list[str] = []
    seen: set[str] = set()
    for consumer_id in (item.strip() for item in raw.split(",")):
        ...
        if not CONSUMER_ID_RE.fullmatch(consumer_id):
            die(
                f"{source}: invalid consumer_id {consumer_id!r}\n"
                "  App consumers must match ^[a-z0-9][a-z0-9_-]{0,61}[a-z0-9]$"
            )
        if consumer_id in BUILTIN_CONSUMER_IDS:
            die(f"{source}: built-in consumer_id {consumer_id!r} is reserved")
        if consumer_id not in declared_consumer_ids:
            die(
                f"{source}: consumer_id {consumer_id!r} was not declared\n"
                f"  Declared consumers: {', '.join(sorted(declared_consumer_ids)) or '(none)'}"
            )
...
def _binding_label(allowed_consumers: list[str]) -> str:
    if allowed_consumers == ["subumbra-proxy"]:
        return "compat/simple"
    return ",".join(allowed_consumers)


def _bind_key_to_consumers(
    key_id: str,
    selected_consumer_ids: list[str],
    *,
    key_consumers_by_key_id: dict[str, list[str]],
    allowed_keys_by_consumer: dict[str, list[str]],
) -> None:
    effective_consumers = selected_consumer_ids or ["subumbra-proxy"]
    key_consumers_by_key_id[key_id] = list(effective_consumers)
    for consumer_id in effective_consumers:
        allowed_keys_by_consumer.setdefault(consumer_id, []).append(key_id)
...
def _validate_allowed_keys(
    ...
    for consumer_id, allowed_keys in allowed_keys_by_consumer.items():
        missing = [key_id for key_id in allowed_keys if key_id not in valid_key_ids]
        if missing:
            die(
                f"{consumer_id} requested unknown allowed key_id(s): {', '.join(sorted(missing))}\n"
                f"  Valid key_ids for this bootstrap run: {', '.join(sorted(valid_key_ids))}"
            )


def _build_consumer_registry(
    consumer_tokens: dict[str, str],
    allowed_keys_by_consumer: dict[str, list[str]],
    *,
    token_ttl_days: int,
    ...
    registry = {
        "subumbra-proxy": {
            "token": consumer_tokens["subumbra-proxy"],
            "allowed_keys": allowed_keys_by_consumer["subumbra-proxy"],
            "can_list_keys": False,
        ...
        "subumbra-ui": {
            "token": consumer_tokens["subumbra-ui"],
            "allowed_keys": [],
            "can_list_keys": True,
            "can_read_stats": True,
    ...
    if "subumbra-probe" in consumer_tokens and "subumbra-probe" in allowed_keys_by_consumer:
        registry["subumbra-probe"] = {
            "token": consumer_tokens["subumbra-probe"],
            "allowed_keys": allowed_keys_by_consumer["subumbra-probe"],
            "can_list_keys": False,
            "can_read_stats": False,
            "can_write_audit": False,
            "issued_at": issued_at,
    ...
    for consumer_id, token in consumer_tokens.items():
        if consumer_id in BUILTIN_CONSUMER_IDS:
            continue
        registry[consumer_id] = {
            "token": token,
            "allowed_keys": allowed_keys_by_consumer[consumer_id],
            "can_list_keys": False,
            "can_read_stats": False,
            "can_write_audit": False,
            "issued_at": issued_at,
...
def compute_policy_hash(policy_doc: dict[str, Any]) -> str:
    ...
    if policy_doc.get("type") == "ssh_key":
        ...
        baseline_allow: dict[str, Any] = {
            "consumers": sorted(allow.get("consumers", [])),
        }
