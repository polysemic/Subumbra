    ...
    *,
    ...
    runtime_env_lines.extend(
        [
            ...
            f"SUBUMBRA_GATE_VAPID_PUBLIC_KEY={gate_vapid_public_key}",
...
def _build_host_env_updates(
    *,
    ...
    host_env_updates = {
        ...
        "SUBUMBRA_GATE_VAPID_PUBLIC_KEY": gate_vapid_public_key,
...
def _normalize_policy_doc(doc: dict[str, Any], source: str) -> dict[str, Any]:
    ...
    if gate is not None:
        if not isinstance(gate, dict):
            _policy_die(source, "gate must be an object")
        ...
        for idx, raw_rule in enumerate(require_approval):
            ...
            adapter = raw_when.get("adapter")
            if adapter is not None:
                adapter = _policy_require_string(adapter, source, f"gate.require_approval[{idx}].when.adapter")
                if not ADAPTER_ID_RE.fullmatch(adapter):
                    _policy_die(source, f"gate.require_approval[{idx}].when.adapter {adapter!r} is invalid")
                normalized_when["adapter"] = adapter
            ...
            unknown_when = set(raw_when) - {"any_request", "adapter", "method", "path_prefix"}
            if unknown_when:
                _policy_die(
                    source,
        ...
        if unknown_gate:
            _policy_die(source, f"gate contains unsupported field(s): {', '.join(sorted(unknown_gate))}")
...
def _normalize_manifest_record(record: Any, idx: int) -> dict[str, Any]:
    ...
    if record_type == "ssh_key":
        ...
        if gate is not None:
            if not isinstance(gate, dict):
                _manifest_die(f"{source}.gate must be an object when provided")
            require_approval = gate.get("require_approval")
            if not isinstance(require_approval, list) or not require_approval:
            ...
            for rule_idx, raw_rule in enumerate(require_approval):
                ...
                if adapter is not None:
                    if not isinstance(adapter, str) or not adapter.strip():
                        _manifest_die(
                            f"{source}.gate.require_approval[{rule_idx}].when.adapter must be a non-empty string"
                        )
                    if not ADAPTER_ID_RE.fullmatch(adapter.strip()):
                        _manifest_die(
                            f"{source}.gate.require_approval[{rule_idx}].when.adapter {adapter.strip()!r} is invalid"
                        )
                    normalized_when["adapter"] = adapter.strip()
                ...
                unknown_when = set(raw_when) - {"any_request", "adapter"}
                if unknown_when:
                    _manifest_die(
            ...
            unknown_gate = set(gate) - {"require_approval"}
            if unknown_gate:
                _manifest_die(f"{source}.gate contains unsupported field(s): {', '.join(sorted(unknown_gate))}")
