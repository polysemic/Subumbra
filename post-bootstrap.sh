...
RUNTIME=$(SUBUMBRA_ADAPTER_REGISTRY=x SUBUMBRA_TOKEN_PROXY=x SUBUMBRA_TOKEN_UI=x SUBUMBRA_TOKEN_PROBE=x SUBUMBRA_HMAC_KEY=x docker compose run --rm -u 0 -T subumbra-keys cat /app/data/runtime.env 2>/dev/null) || {
