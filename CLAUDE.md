...
| **Prompt-injection capability abuse** | An LLM-mediated app can be manipulated into making requests that exceed its intended scope (e.g. a chat app making payment API calls). | `capability_class` + `allow.methods`/`path_prefixes` prevent cross-capability use at the Worker boundary. `intent.trust` (R46-1) adds initiator and content-source gating. |
...
| **Response-side exfiltration** | The Worker (or a replaced Worker) could inspect or redirect upstream API responses before returning them to the adapter. | `response.deny_patterns` (reserved now; enforced in R46-1) will scan upstream responses for credential-like content. |
