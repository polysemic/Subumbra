...
- **R64 close-out (2026-05-13):** Round **CLOSED**; archive `council/closed/r64-launch-polish/`. Official VPS `existing-stack` **PASS** run: `gemini-vps-20260513T005931Z` (with `--build subumbra-keys subumbra-ui`). **`claude-verification.md`** / **`codex-verification.md`** not filed — process gap; restore three-LLM verification when practical.
- **round-cleanup `verify-round.sh`:** Static greps (S1, S2, S6–S8) plus live curls for proxy `/health` (reject `worker_auth: stale`), UI `/` headers (CSP + `no-store`), Worker `HEAD /health` (200). Reads `CF_WORKER_URL` and host ports from repo `.env` (`_env_val()`), not `docker compose exec` (allowed variance vs plan wording). Approved plan: `council/approved/round-cleanup.md`.
- **round-cleanup close-out (2026-05-13):** Round **CLOSED**; archive `council/closed/round-cleanup/`. Official VPS `existing-stack` **PASS**: `codex-vps-20260513T143105Z`, `Gemini-vps-20260513T142722Z` (SHA `05083d1`). **`claude-verification.md`** not filed. Codex verification noted **scope deviation** (UI/README/install `worker_auth` follow-through beyond approved file list); council accepted as non-blocking — see **`council/approved/round-cleanup.md` § Close-Out**.
- **R71 close-out (2026-05-17):** Round **CLOSED**; archive `council/closed/r71-security-hardening/`. Product/security evidence came from Gemini’s full reinstall + LiteLLM path in `gemini-verification-2.md` and Claude’s manual adversarial probe pass in `claude-verification-2.md`; both accepted. Practical verifier gotcha: live `keys.json` may show active `policy.velocity` on keys while `template_name` is unset, so round hooks should not key Scenario-9-style checks exclusively on `template_name`. Harness note: `scripts/council/vps-proof-run.sh --deploy-worker` now needs to inject the live KV namespace ID from `kv-config.json` into the temporary Wrangler config before deploy.
