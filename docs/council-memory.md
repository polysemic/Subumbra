...
- **R62 `verify-round.sh`:** Static checks on `bootstrap/subumbra-bootstrap.py` (8-tuple wizard unpack, `_WIZARD_SECRETS` + `_resolve_manifest_secret` order, Step 10 scrub) plus optional host `curl` proxy `/health` when port reachable; full TTY wizard smoke remains operator/harness outside the hook (`council/approved/r62-interactive-bootstrap.md`).
