...
- The app receives an adapter token and a proxy URL. Its config never contains a provider key. A leaked config leaks a scoped token, not a provider credential. [src:subumbra-proxy]
...
- Rotation can re-encrypt an existing provider secret against the on-disk public key without changing the app's adapter token or touching Cloudflare. [src:subumbra-rotation]
