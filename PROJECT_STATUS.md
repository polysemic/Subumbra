...
*Rounds 1–43.6 closed. Round 43-6-1 is next. See `council/COUNCIL.md` for round history and current status.*
...
2. **Round 43-6-1 (Queued)** — Env ingestion script, `subumbra-ui` health endpoint, multi-provider template updates, `0.0.1-alpha` version bump. Light code round. Scope is limited to multi-app ingestion plus shared-key deduplication under the current bootstrap contract; richer same-provider multi-secret import support is deferred.

3. **Round 43-6-2 — Per-App Identity Enforcement (Planned)** — Activate the
   per-app allowed key lists generated by the 43-6-1 ingestion script through a
   future secure-gateway cutover in `subumbra-proxy`. Delivers: per-app request
   attribution in audit trail and UI logs; per-app key scope enforcement; later
   retirement of the flat `PROXY_ALLOWED_KEYS` union. Deferred to later rounds:
   key rotation per app, adding/removing key_ids from an app's allowed list
   post-bootstrap, per-app rate limiting, and UI-based scope management.

4. **Round 44 (Planned)** — Secure UI round. UI-based env ingestion, encrypted paste/input for browser security. See `council/round-44-secure-ui/`.
