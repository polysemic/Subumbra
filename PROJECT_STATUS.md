# PROJECT_STATUS
*Current state — updated 2026-05-17 (1.0.0-alpha Public Release + R71 Security Hardening — READY)*
...
1. **Strategic roadmap**: Planned and possible work lives in the root [`ROADMAP.md`](ROADMAP.md). Themes there are grouped for scheduling—**not** a committed sequence. `r72-cloudflare-runtime-ux` completed Cloudflare as a polished optional deployment path (BYOC Tunnel and Access, day-2 credential management). `r73-cloudflare-autoprovision` remains planned but not yet scheduled — see the `ROADMAP.md` appendix. Cloudflare is no longer an active theme; future work is debug- or request-driven only.
...
9. **Completed Cloudflare arc (r72):** `r72-cloudflare-runtime-ux` delivered Cloudflare BYOC operator inputs, day-2 credential rotation, and updated docs. `r73-cloudflare-autoprovision` (API-driven Tunnel and Access provisioning) is planned but not yet approved — it requires a new `cf-api-provision` verification lane definition before the approved plan can be written.
