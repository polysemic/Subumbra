# PROJECT_STATUS
*Current state — updated 2026-05-23 (r83-multi-session closed; local, VPS, and main synced at `8febbac`)*
...
13. **Deferred from r84-ssh-vault (2026-05-24):** `confirm_each_sign` interactive enforcement (requires agent or human-in-the-loop gate) and a local SSH agent daemon are r85-priority items. Bootstrap monolith modularization (session, CF, keys, adapters) is r86-priority. Passphrase-protected OpenSSH Ed25519 key ingestion (bcrypt dependency) is a follow-up once the operator dependency story is clear. RSA SSH (rsa-sha2-256/512), ECDSA, GPG/git-commit signing, SSH CA, and GitHub Apps JWT are low-priority future capabilities.
