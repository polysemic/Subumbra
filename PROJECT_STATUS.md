...
0. **Bootstrap UX Redesign (Council-Tracked — High Priority)**
   Two related blockers identified during 2026-05-09 full-install run. Both require approved council scope before implementation:
   - **`bootstrap.sh` as nuke-and-pave**: `bootstrap.sh` should be a destructive fresh-start that also tears down live Cloudflare state (Durable Object vault instances, KV namespace) with operator acknowledgement before proceeding. Commands like `--rotate` and `--push-registry` are the day-2 operations. Today, a "full reset" wipes Docker volumes but leaves CF vaults intact, causing every re-bootstrap to hit HTTP 409 "already initialized" on all shared-vault keys. The core bootstrap loop also has no fallback for the already-initialized case — `--provision` repair is the only path and requires a code fix (see [bootstrap/subumbra-bootstrap.py](bootstrap/subumbra-bootstrap.py) `run_provision_key()`). The approved UX model should make `bootstrap.sh` the single command an operator reaches for when starting over, with CF teardown included.
   - **`.env.bootstrap` shred-per-invocation breaks multi-key repair**: `bootstrap.sh` shreds `.env.bootstrap` after every successful invocation, including `--provision`. Running `--provision` sequentially for twelve keys requires re-supplying credentials before each call. Either a `--no-shred` flag, a multi-key `--provision-all` mode, or deferred shredding until the last key is written would make the repair workflow operator-usable without external scripting.
   - **`deny_patterns` safe vocabulary and validation UX**: the current anchored-literal restriction prevents substring-style response scanning use cases, and bootstrap error messaging for invalid patterns needs a more operator-guided explanation. This is staged for `r48-6-bootstrap-ux`, not R48-5 close-out.
