...
- **Round 45-4 — Worker REST Enforcement Foundation** (Remediation complete 2026-05-04; pending re-verification): `SUBUMBRA_ADAPTER_TOKENS` now carries `{id, token}` objects so the Worker resolves adapter identity from the authenticated token; `getRegistryEntry` now returns the full `allow` block; `handleProxy` enforces adapter scope, method, path prefix, content-type, and body size for V3 records (V2 grace path skips all allow-block checks); `authorization`, `x-api-key`, and `x-api-key-id` are now stripped by `HOP_BY_HOP_HEADERS`; structural `intent` metadata (`source`, `trust`) is logged when present; `docs/adapter-contract.md` forward claim corrected to R45-4. Codex initial clean-run failed on fixture gaps (`SUBUMBRA_POLICY_FILE_1` unrecognised → `test_proxy_key` not provisioned); Gemini passed V1–V5/V7–V8, flagged V6 as environmental limitation. Remediation: `policies.json` added in correct format, `bootstrap-overlay.env` updated to use `SUBUMBRA_POLICY_PATH` + key slots 3/4, `verify-round.sh` V6 made optional (SKIP when no V2 key). Claude verification report confirms C1–C10 all pass and no source code bugs. Clean-run hook expected to pass V1–V5, V7–V8 and SKIP V6.
