# PROJECT_STATUS
*Current state — updated 2026-05-21 (r76-response-injection-fix close-out)*
...
11. **Deferred from r76-response-injection-fix (2026-05-21):** Response-header whitelist/blacklist policy and request-header passthrough remain out of scope for `r76` and should be handled in a dedicated follow-up round (`r77-response-header-policy`). Raw query-string normalization in `subumbra-proxy/app.py` also remains deferred until targeted compatibility proof confirms a safe remediation path.
