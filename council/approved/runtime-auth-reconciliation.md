...
Date: 2026-04-19
Status: Approved
Based on: `claude-investigation.md` (resolves all blocking questions)
Consensus reviewers: Claude, Codex, Gemini (claude-review.md, codex-review.md,
gemini-review.md, claude-review-2.md, codex-review-2.md, gemini-review-2.md,
gemini-investigation.md, claude-investigation.md)
...
In the `litellm:` service `environment:` block (lines 98-110 in current file),
...
# SUBUMBRA_PROVIDER_PREFIXES='{"my_provider":"/api/v2"}'   ← remove comment too
```
...
In the token drift check loop (line 92), remove `litellm` from the service list.
...
Also remove the `litellm)` case from the inner `case "$svc"` block (around
lines 94-99). The relevant lines to remove:
```bash
